...
Run the two blocks below to setup the Rmarkdown script. The first block (setup) installs and loads the required libraries. The second block (read-files) reads all the data we're going to be analyzing into data frames.
...
```{r species-tree-mm, out.width="50%", fig.align = "center", warning=FALSE, fig.height=8, fig.width=10}
...
# Count the number of M1 elements
...
With these cutoffs, we count 784 out of our 2000 elements being accelerated for marine mammals. That is a lot, but please remember this is NOT a random sampling of loci. This sample includes many of the elements originally found to be accelerated in marine mammals and the total dataset has over 200,000 elements.

And also remember we are still working with loci run with the species tree model, so phylogenetic discordance has not been accounted for.
...
```{r tree-comps-st, out.width="50%", fig.align = "center", warning=FALSE, fig.width=12, fig.height=16}

#st_results_m1 = subset(st_results, best.fit.model=="M1")
#m1_locus = sample_n(st_results_m1, 1)
# This randomly selects an M1 locus

m1_locus = subset(st_results, phyloacc.id=="73-15")
...
The top two rows show trees from the same locus with branches and conservation states under M1 (left) and M2 (right). The model that best fits this locus is M1. You can see that, when we allow acceleration on any branch with M2, many branches are inferred to be in the accelerated state, including all of the marine mammal branches. However, the probability of this model is lower than the probability of accelerations occurring only on marine mammal lineages, so we select that as the model for this locus and we can further investigate whether this element may play a role in the aquatic phenotype.
...
# Accelerated elements per branch with M2

For example, we can count how many times each branch is inferred to be accelerated across all elements. For this we use a third Bayes factor:
...
```{r accel-counts-st-m2, out.width="50%", fig.align = "center", warning=FALSE, fig.height=8, fig.width=8}
...
names(st_m2_counts) = c("Branch", "# accelerate loci under M2")
...
```{r species-tree-control-st, out.width="50%", fig.align = "center", warning=FALSE, fig.height=8, fig.width=8}
...
So here we can pick out clades sister to marine mammals that don't share our traits of interest (those relating to living in water) and count the number of accelerated elements found uniquely in their ancestral branches.
...
Here we find only one element, so if our question was whether the transition to water led to excessive amounts of convergence... well maybe for this dataset.
...
```{r species-tree-echo, out.width="50%", fig.align = "center", warning=FALSE, fig.height=8, fig.width=8}
...
Run the block below to count the number of elements accelerated in ancestral echolocating lineages:
...
for(i in 1:nrow(st_results_m1_m2)){
  ...
  # set of ancestral mecholocating branches
    ...
    # Check if all the accelerated lineages in the current locus are echolocating lineages
      ...
      st_results_echo_anc = rbind(st_results_echo_anc, st_results_m1[i,])
...
When estimating substitution rates, the use of a single species tree on alignments from loci that have a different phylogeny results in the mis-mapping of substitutions and the mis-inference of rates. Downstream affects could include spurious inferences of rate acceleration or positive selection. PhyloAcc's gene tree model accounts for discordance caused by **incomplete lineage sorting** by sampling phylogenies for each locus and then estimating rates.

But what practical effect does this have on results?

## Quantifying discordance

The PhyloAcc pipeline now quantifies discordance using **site concordance factors (sCF)**. That is, for each locus it counts the number of sites that support the topology by sampling quartets around internal branches. We then use this to decide which model to run the locus with, the species tree model or the gene tree model.

Run this block of code to display average site concordance factors across all loci:

```{r scf-tree, out.width="75%", fig.align = "center", warning=FALSE, fig.height=8, fig.width=8}
...
#high_locus = subset(st_results, original.id.st=="chr5-87")
...
st_results_low = subset(all_results, node.scf.avg < 0.4)
...
#low_locus = subset(st_results, original.id.st="chr18-5")
...
cur_title = paste("Locus: ", high_locus$original.id, " / Avg. sCF: ", round(high_locus$node.scf.avg,3), " / Displayed model: ", high_locus$best.fit.model.st, sep="")
...
cur_title = paste("Locus: ", high_locus$original.id, " / Avg. sCF: ", round(high_locus$node.scf.avg,3), " / Displayed model: ", high_locus$best.fit.model.gt, sep="")
...
cur_title = paste("Locus: ", low_locus$original.id, " / Avg. sCF: ", round(low_locus$node.scf.avg,3), " / Displayed model: ", low_locus$best.fit.model.st, sep="")
...
cur_title = paste("Locus: ", low_locus$original.id, " / Avg. sCF: ", round(low_locus$node.scf.avg,3), " / Displayed model: ", low_locus$best.fit.model.gt, sep="")
